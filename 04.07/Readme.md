# Karqo Sistemi:
1. `OrderStatus` adında enum yaradılmalıdır. Alacağı dəyərlər (Created, OnTheWay, Delivered, Cancelled)
2. Yaradılmalı olan class-lar aşağıdakılardır. Mütləq şəkildə instance-lar `constructor` ilə yaradılmalıdır
    - `Customer`
    ...
    - `CargoOrder`
        ...
        - `UpdateStatus()` metodu:
            Bu metod `OrderStatus` tipindən bir dəyər qəbul edəcək. Sifarişin statusunu yeniləmək üçün istifadə olunacaq. Lakin burda əlavə yoxlama aparılacaq ki, əgər sifariş ləğv edilibsə və ya tamamlanıbsa statusu dəyişmək mümkün olmasın. 
3. `ICargoService` interfeysi:
    Özündə statik olaraq `Customer`, `Courier` və `CargoOrder` massivləri saxlayacaq
